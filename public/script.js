document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', function(e) {
      ...
      if (isValid) {
        // Form is valid - you can submit via fetch or allow default
    ...
    contactForm.addEventListener('blur', function(e) {
      ...
      if (!field.matches('input, textarea, select') || !field.hasAttribute('required')) return;
      
      const value = field.value.trim();
      const existingError = field.parentElement.querySelector('.form-error');
      
      if (existingError && value) {
        if (field.type === 'email') {
          const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
          if (emailRegex.test(value)) {
  ...
  if (fadeElements.length > 0) {
    ...
    if (!('IntersectionObserver' in window)) {
      ...
      let fadeScrollTimeout;
      window.addEventListener('scroll', function() {
        if (fadeScrollTimeout) {
          window.cancelAnimationFrame(fadeScrollTimeout);
        }
        fadeScrollTimeout = window.requestAnimationFrame(checkFadeElements);
      }, { passive: true });
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '16624bfb-f186-49b4-965c-b0dffb8882a5',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '16624bfb-f186-49b4-965c-b0dffb8882a5';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '16624bfb-f186-49b4-965c-b0dffb8882a5';
